...
if [ -z "$model_name_or_path" ]; then
    model_name_or_path="<pretraining checkpoint path>"
fi
...
trainable_modules="visual.merger,model"

if [ -z "$DATA_CONFIG_FILE" ]; then
    DATA_CONFIG_FILE="train/data_configs/data_llava_ov_video_packed.yaml"
fi
OUTPUT_DIR="output/instruction_tuning"
if [ -z "$RUN_NAME" ]; then
    RUN_NAME="vamba_llava_video_ov_packed_lr1e-7"
fi

export WANDB_API_KEY="<YOUR WANDB API KEY>"
export WANDB_PROJECT="vamba_instruction_tuning"
